...
//            BlockChainShellQueueUtils.add(bcCluster);
            log.info("队列的长度为：{}", BlockChainShellQueueUtils.size());
        }
    }
...
//            BlockChainShellQueueUtils.add(bcCluster);
        ...
        log.info("队列的元素为：{}", bcCluster.getClusterName());
        bcCluster = (BCCluster) BlockChainShellQueueUtils.peek();
        log.info("队列的第一个元素为：{}", bcCluster.getClusterName());
    ...
    public void testQueueWait() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ThreadPoolManager.newInstance().addExecuteTask(new Runnable() {
                ...
                public void run() {
                    try {
                        log.info("获取任务队列长度{}", BlockChainShellQueueUtils.size());
